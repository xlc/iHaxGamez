...
- (NSMutableArray *)getSearchArray:(Byte *)Value ByteSize:(int)Bytes SoughtValueString:(NSString *)ValueString PrgBar:(NSProgressIndicator *)pBar dataType:(int)type;
{
					...
					while(destPosition <= endDestPosition)
					{
						...
						if (destPosition[0] == Value[0])
						{
							...
							do
							{
								...
								{
									[AddrList addObject:[[[AppAddressData alloc] initWithValues:(vm_address_t)(SourceAddress + (resetDestPosition - ReturnedBuffer)) val:ValueString dataType:type] autorelease]];
									break;
								}
