...
- (id)initWithAppName:(NSString *)AppName PID:(pid_t)PID
{
    ...
    if (self)
    {
		...
		[tblResults setDoubleAction:@selector(AddAddress:)];
		
		[self setAddressListDS:[[[AddressListDataSource alloc] init] autorelease]];
		[addressListDS setSearchWindowController:self];
		[tblAddressList setDataSource:addressListDS];
...
{
    return addressListDS;
}

- (void)setAddressListDS:(AddressListDataSource *)newAddressListDS
{
	if (addressListDS != newAddressListDS)
	{
		[addressListDS release];
		addressListDS = [newAddressListDS retain];
	}
}
...
{
	NSUInteger AddrCount = [[addressListDS appAddresses] count];
	...
	for (x=0; x<AddrCount ; x++)
	{
		MyAppAddr = (AddressListData *)[[addressListDS appAddresses] objectAtIndex:x];
		
		SelectedIndex = [MyAppAddr type];
...
{
	if ([sender isKindOfClass:[MyTableView class]])
	{
		AppAddressData* rowData = [[appAddressDS appAddresses] objectAtIndex:[tblResults selectedRow]];
...
{
	[addressListDS removeAllObjects];
	[tblAddressList reloadData];
}
...
- (void)searchAndFilter:(bool)isFilterMode
{
    ...
    else
    {
        ...
        [appAddressDS setAppAddresses:[AttachedMemory getSearchArray:searchValPointer ByteSize:searchValSize SoughtValueString:ValueString PrgBar:progressBar dataType:SelectedIndex]];         
    }
